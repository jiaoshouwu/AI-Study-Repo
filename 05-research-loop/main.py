...
def run_research(topic: str) -> str:
    ...
    trace.append({"stage": "initial_research", "questions": len(questions)})

    evidence = collect_evidence(questions)

    searched_questions = {item["question"].strip().lower() for item in evidence}
    ...
    while True:
        ...
        normalized_query = next_query.strip().lower()

        if normalized_query in searched_questions:
            print("Duplicate research query detected. Stopping.")

            break

        searched_questions.add(normalized_query)

        result = research_question(next_query)

        trace.append({"stage": "extra_search", "query": next_query})

        evidence.append({"question": next_query, "result": result})
    ...
    weak_evidence = [
        {
            ...
            "result": ("An AI agent can use models and tools"),
        }
    ]
